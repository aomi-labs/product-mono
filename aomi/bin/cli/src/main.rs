...
impl From<BackendSelection> for BackendType {
    fn from(value: BackendSelection) -> Self {
        match value {
            ...
            BackendSelection::Polymarket => BackendType::Polymarket,
        }
    }
}
...
async fn run_interactive_mode(
    ...
    println!("Commands: :help, :backend <default|l2b|polymarket>, :exit");
    print_prompt()?;
...
async fn handle_repl_line(
    ...
    if trimmed == ":help" {
        ...
        println!("  :backend <name>        Switch backend (default, l2b, polymarket)");
    ...
    if let Some(rest) = trimmed.strip_prefix(":backend") {
        ...
        if backend_name.is_empty() {
            println!("Usage: :backend <default|l2b|polymarket>");
            return Ok(ReplState::ImmediatePrompt);
        }
        ...
        match BackendSelection::from_str(backend_name, true) {
            ...
            Err(_) => {
                println!("Unknown backend '{backend_name}'. Options: default, l2b, polymarket");
            }
        }
        return Ok(ReplState::ImmediatePrompt);
    }
...
async fn build_backends(
    ...
    let polymarket_app = Arc::new(
        PolymarketApp::new_with_options(no_docs, skip_mcp)
            .await
            .map_err(|e| anyhow::anyhow!(e.to_string()))?,
    );
