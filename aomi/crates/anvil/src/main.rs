...
fn resolve_providers_path() -> Result<PathBuf> {
    ...
    let candidates = [PathBuf::from("providers.toml"), PathBuf::from("../providers.toml")];
...
mod tests {
    ...
    async fn test_spawn_multiple_forks_across_chains() -> Result<()> {
        ...
        let key = alchemy_key();
        let config = config_for_instances(vec![
            ("ethereum", 1, format!("https://eth-mainnet.g.alchemy.com/v2/{}", key)),
            ("optimism", 10, format!("https://opt-mainnet.g.alchemy.com/v2/{}", key)),
            ("arbitrum", 42161, format!("https://arb-mainnet.g.alchemy.com/v2/{}", key)),
            ("base", 8453, format!("https://base-mainnet.g.alchemy.com/v2/{}", key)),
        ]);
