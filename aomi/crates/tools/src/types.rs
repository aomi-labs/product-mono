...
// AomiTool through the generic implementation below.

// Generic implementation of AomiTool for Rig tools
impl<T> AomiTool for T
where
...
pub trait AnyTool: Send + Sync {
    ...
    fn validate_async_result(&self, value: &Value) -> EyreResult<Value> {
        Ok(value.clone())
    }
}
...
/// Adapter to register multi-step tools without conflicting with the blanket AomiTool impl.
#[derive(Clone)]
pub struct AsyncToolWrapper<T: AsyncTool + Clone + 'static> {
    pub inner: T,
}

impl<T> AnyTool for AsyncToolWrapper<T>
where
    T: AsyncTool + Clone + 'static,
{
    ...
    fn validate_async_result(&self, value: &Value) -> EyreResult<Value> {
        self.inner.validate_async_result(value)
    }
}
    ...
    T::ApiResponse: Serialize + Send + 'static,
{
    ...
    fn tool(&self) -> &'static str {
        <T as AomiTool>::name(self)
    }

    fn description(&self) -> &'static str {
        <T as AomiTool>::description(self)
    }

    fn static_topic(&self) -> &'static str {
        let original = <T as AomiTool>::static_topic(self);
...
mod tests {
    ...
    {
        let response = AomiTool::call(tool, request).await?;
        Ok(response)
    }
    ...
    async fn name_description_and_validation_forward() {
        ...
        assert_eq!(AomiTool::name(&tool), GetCurrentTime::NAME);
        assert_eq!(AomiTool::description(&tool), GetCurrentTime::NAME);
        assert!(AomiTool::check_input(&tool, args.clone()));

        let output = AomiTool::call(&tool, args)
            .await
