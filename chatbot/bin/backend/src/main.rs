...
mod tests {
    ...
    async fn test_session_manager_create_session() {
        ...
        let session_state =
            session_manager.get_or_create_session(session_id, true).await.expect("Failed to create session");
    ...
    async fn test_session_manager_multiple_sessions() {
        ...
        let session1_state =
            session_manager.get_or_create_session(session1_id, true).await.expect("Failed to create session 1");

        let session2_state =
            session_manager.get_or_create_session(session2_id, true).await.expect("Failed to create session 2");
    ...
    async fn test_session_manager_reuse_session() {
        ...
        let session_state_1 =
            session_manager.get_or_create_session(session_id, true).await.expect("Failed to create session first time");

        // Get session second time
        let session_state_2 =
            session_manager.get_or_create_session(session_id, true).await.expect("Failed to get session second time");
