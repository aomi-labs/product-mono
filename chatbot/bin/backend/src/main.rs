...
fn determine_allowed_origins() -> AllowedOrigins {
    let candidate_values = if let Ok(raw) = env::var("BACKEND_ALLOWED_ORIGINS") {
        raw.split(',').map(|entry| entry.trim().to_owned()).filter(|entry| !entry.is_empty()).collect::<Vec<_>>()
    } else {
        default_origin_candidates()
    };

    let mut normalized: Vec<String> =
        candidate_values.into_iter().filter_map(|value| normalize_origin(&value)).collect();
...
fn default_origin_candidates() -> Vec<String> {
    let mut origins = vec!["http://localhost:3000".to_string(), "http://127.0.0.1:3000".to_string()];
