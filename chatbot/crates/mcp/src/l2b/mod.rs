...
use rmcp::{ErrorData, model::CallToolResult, handler::server::tool::Parameters};
...
fn deserialize_handler<'de, D>(deserializer: D) -> Result<lib::handlers::config::HandlerDefinition, D::Error>
where
    D: Deserializer<'de>,
{
    use serde::de::Error;
...
pub struct AnalyzeContractParams {
    #[schemars(description = "Ethereum contract address (e.g., '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48')")]
    pub contract_address: String,

    #[schemars(description = "Natural language description of what data to extract from the contract (e.g., 'find implementation and admin', 'get all roles and members')")]
...
pub struct ExtractDataParams {
    ...
    #[schemars(description = "Handler configuration specifying how to extract data (can be JSON object or JSON string)")]
...
impl L2BTool {
    ...
    pub async fn analyze_contract(
        ...
        let runner =
            DiscoveryRunner::new(Network::Mainnet, provider).expect("Failed to create DiscoveryRunner");
        ...
        let (contract_analysis, handlers) = runner
            ...
            .await
            .map_err(|e| ErrorData::internal_error(format!("Failed to analyze contract: {}", e), None))?;

        // Build response JSON
        let response = serde_json::json!({
            "pattern_detected": contract_analysis.pattern_detected,
            "summary": contract_analysis.summary,
            "handlers": handlers.into_iter().map(|(name, def)| {
                serde_json::json!({
                    "field_name": name,
                    "handler": def
                })
        ...
        2. When displaying handlers, show them as formatted from the output so that the user will be able to directly execute if they wish.");
    ...
    async fn execute_handler_internal(
        ...
        if let Some(expected) = expected_type {
            ...
            if actual_type != expected {
                return Err(ErrorData::invalid_params(
                    format!("Handler must be of type '{}', got '{}'", expected, actual_type),
                    None,
                ));
            }
        }
        ...
        let contract_address = Address::from_str(&params.contract_address)
            .map_err(|e| ErrorData::invalid_params(format!("Invalid contract address: {}", e), None))?;

        // Create provider
        let provider = RootProvider::<AnyNetwork>::new_http("http://localhost:8545".parse().unwrap());
        let runner = DiscoveryRunner::new(Network::Mainnet, provider)
            .map_err(|e| ErrorData::internal_error(format!("Failed to create runner: {}", e), None))?;
        ...
        let result = runner
            ...
            .await
            .map_err(|e| ErrorData::internal_error(format!("Failed to execute handler: {}", e), None))?;
