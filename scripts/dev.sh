...
BACKEND_RUST_LOG="${RUST_LOG:-info}"
for _ in {1..5}; do
  if [[ -n "${NO_PROXY:-}" && -n "${no_proxy:-}" ]]; then
    echo "🔧 Starting backend with NO_PROXY: $NO_PROXY and no_proxy: $no_proxy"
    NO_PROXY="$NO_PROXY" no_proxy="$no_proxy" RUST_LOG="$BACKEND_RUST_LOG" cargo run -p backend -- --no-docs & BACKEND_PID=$!
  else
    RUST_LOG="$BACKEND_RUST_LOG" cargo run -p backend -- --no-docs & BACKEND_PID=$!
  fi
  sleep 2
