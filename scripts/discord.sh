...
REQUIRED_KEYS=("DISCORD_BOT_TOKEN" "ANTHROPIC_API_KEY")
MISSING_KEYS=()
...
# Discord config (show current settings)
echo ""
echo "💬 Discord Configuration:"
echo "   DM Policy: ${DISCORD_DM_POLICY:-open}"
echo "   Guild Policy: ${DISCORD_GUILD_POLICY:-mention}"
if [[ -n "${DISCORD_ALLOW_FROM:-}" ]]; then
  echo "   Allowlist: ${DISCORD_ALLOW_FROM}"
fi
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo "  DISCORD_BOT_TOKEN      (required) Bot token from Discord Developer Portal"
      ...
      echo "  DISCORD_DM_POLICY      (optional) open|allowlist|disabled (default: open)"
      echo "  DISCORD_GUILD_POLICY   (optional) mention|always|disabled (default: mention)"
      echo "  DISCORD_ALLOW_FROM     (optional) Comma-separated user IDs for allowlist"
      exit 0
      ;;
    *)
...
cargo build -p aomi-discord $BUILD_FLAGS
...
RUST_LOG="${RUST_LOG:-info}" cargo run -p aomi-discord $BUILD_FLAGS -- $RUN_FLAGS
