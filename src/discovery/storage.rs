...
impl StorageSlot {
    ...
    pub async fn get_resolved_value<N: Network>(&self, provider: &RootProvider<N>, address: &Address) -> Result<U256, String> {
    ...
    pub fn convert_return(&self, storage_value: U256) -> Result<HandlerValue, String> {
        ...
        match return_type.as_ref().map(|s| s.as_str()) {
            ...
            Some("bytes") | None => {
                Ok(HandlerValue::Bytes(Bytes::copy_from_slice(&bytes)))
            }
            Some("number") => {
...
impl<N> StorageHandler<N> {
    ...
    pub fn from_handler_definition(field: String, handler: HandlerDefinition) -> Result<Self, String> {
        match handler {
            HandlerDefinition::Storage { slot, offset, return_type, ignore_relative } => {
                let slot_val = slot
